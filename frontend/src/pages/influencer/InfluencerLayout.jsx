...
import {
  ...
  Handshake, MessageCircle, ScrollText,
} from "lucide-react";
      ...
      <main className="flex-1 min-w-0 flex flex-col">
        ...
        <div className="h-16 border-b border-border flex items-center px-4 md:px-8 gap-3">
          ...
          <div className="ml-auto flex items-center gap-2">
            <NotificationBell />
            <div className="h-9 w-9 rounded-full bg-primary text-white text-sm font-semibold flex items-center justify-center" data-testid="influencer-avatar">
              {initials}
            </div>
          </div>
        </div>
        ...
        <nav className="md:hidden fixed bottom-0 inset-x-0 z-30 border-t border-border bg-card flex justify-between px-2 py-1" data-testid="influencer-mobile-nav">
          {NAV.map((item) => (
            <NavLink
              key={item.to}
              to={item.to}
